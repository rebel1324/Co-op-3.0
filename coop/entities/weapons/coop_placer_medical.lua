...
if SERVER then
	function SWEP:PrimaryAttack()
		...
		dispencer = ents.Create("coop_healer")
		dispencer:SetPos(trace.HitPos:GridClamp(gridsize) + trace.HitNormal*1 )
		dispencer:SetAngles(trace.HitNormal:Angle() - Angle(0, 0, 0))
		dispencer:Spawn()
		dispencer:Activate()

		EntSaveTable("coop_healer")
	end

	function SWEP:SecondaryAttack()
		local trace = self.Owner:GetEyeTraceNoCursor()

		if trace.Entity and trace.Entity:IsValid() and trace.Entity:GetClass() == "coop_healer" then
			trace.Entity:Remove()

			timer.Simple( .01, function() EntSaveTable("coop_healer") end)
		end
		return false
	end
end
...
if CLIENT then
	function SWEP:Deploy()
		...
		self.ghost = ClientsideModel("models/props_combine/health_charger001.mdl")
